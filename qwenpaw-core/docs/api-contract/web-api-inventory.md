...
| `/workspace/checkpoints` | `routers/checkpoints.py` | Checkpoint create/list/restore | Full unchanged-Console compatibility: all 11 calls provide durable per-Workspace automatic/manual/safety snapshots, graph and HEAD metadata, Thread conversation plus Memory/selective-file preview and restore, rollback safety, retention settings, GC/compaction and reset. Archives are bounded, integrity-checked and protected against traversal and links; state survives Rust Desktop restarts. Multi-root checkpoint coordination remains deferred |
...
| `/cron` | `app/crons/api.py` | Job CRUD, pause/resume/run/history | Partial compatibility: unchanged Console CRUD, pause/resume, dispatch-target lookup, persisted state/history, immediate Console text delivery and optional Inbox result capture are implemented and exercised through the original UI; background scheduling, Agent execution, non-Console channels and Heartbeat remain deferred |
