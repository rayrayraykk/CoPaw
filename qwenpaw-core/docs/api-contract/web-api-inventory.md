...
| `/local-models` | `routers/local_models.py` | Local model lifecycle/download | Full unchanged-Console route compatibility for the current Local Model Manage contract: platform-specific pinned llama.cpp runtime download/update, safe bounded archive extraction, Hugging Face/ModelScope GGUF download, progress/cancel state, recommendation and installed-model discovery, real `llama-server` lifecycle/readiness, `qwenpaw-local` activation, stop/delete and Core restart recovery. In-flight cancellation, dual-model switching and unexpected-child-exit browser regression remain explicit follow-up gates |
