...
| Model provider | Provider manager and many adapters | Partial: native OpenAI-compatible Chat Completions, Responses, Anthropic Messages and Gemini GenerateContent; selected Provider/Agent options drive Rust execution, with local tool/history/image fixtures | Required providers, auth methods, model metadata, retry/rate-limit behavior and advanced media/reasoning have parity tests; native protocol support alone is not complete provider parity |
...
| Tool approvals | Python governance/access control | Partial: one-time App Protocol/Console actions; durable Agent/session/root identity, global Inbox polling and root-filtered approval list. Original Inbox reload, cross-Agent approve/deny and real file effects pass without switching the selected Agent. Corrupt identity data denies the tool; direct Console thread IDs cannot cross Agent ownership | Persistent/generalized policies, delegated execution-owner distinction, lifecycle routing, audit, timeout, recovery and all-client parity are covered |
...
| MCP OAuth | Python browser OAuth routes/store | Rust Core discovery, PKCE/loopback callback, secure credential persistence/refresh/revocation, unchanged Console contract, and App Protocol/VS Code flow | Native credential-store and provider interoperability pass on every release target |
| Workspace identity | Python Workspace/project-directory services | Partial: canonical Thread root, Desktop project selection and idle-Thread rebinding; native registry v3 separates retained directory generations from reusable Agent IDs. Runtime Workspace/config/model/project reads use one validated registration snapshot and reject mismatched roots without repairing them | Cron and ChatCatalog now persist typed ownership, including scoped restore; ordinary Console now snapshots Agent policy and drains its runs on disable/delete, independently of SSE backpressure. Usage/ACL and checkpoint storage remain incomplete. Entry-time validation is not a lease against later filesystem replacement. Multi-root and native platform parity remain required |
...
| Non-secret configuration | Python config APIs/files | Partial: base URL/default model, Desktop UI language, preferred Workspace, and Coding Mode in Core SQLite | Remaining settings have real Rust runtime semantics; schema upgrades, precedence, export/restore, and rollback rules pass |
...
| Agents and multi-agent isolation | Python agent manager/scoped routers | Partial: native registry/workspace CRUD and original page controls; Copy preserves native task specifications/public IDs with fresh internal keys and ordinary-failure rollback. Global Inbox approval routing uses persisted ownership. Public scoped Cron HTTP/scheduling now resolve validated Workspace bindings, including retained-root re-registration and reused-name isolation; lifecycle stop/drain and re-enable state rebuilding remain shared with the existing Agent controls | All scoped routes, multi-agent orchestration, per-agent settings, approval/lifecycle semantics and installed-client isolation pass |
...
| Cron/background work | Python cron services | Partial: native time engine, persisted cursors and scheduled/manual Console text/Agent execution. Public Job HTTP now checks the effective Agent binding and resolves WorkspaceDataKey + public ID; PUT is namespace-local create-or-replace. Healthy copied jobs schedule; invalid/disabled/unregistered owners never fall back to default. Deferred manual admission retains the original internal identity across restore. Cron v4 ownership, scoped backup/restore, durable sessions, per-turn safety, queue/cancel/drain, trace/Inbox and retained-root re-registration remain authoritative. Original writer CRUD/tools/history/reload and unchanged default data pass; final suite evidence is tracked in [public Cron acceptance](../testing/cron-public-scope-acceptance.md). Existing `737ANs` packages do not contain this change | All Agent/session/provider/control semantics, external channel delivery, CLI/TUI/SDK operations, cross-platform install/restart and unchanged-UI acceptance pass |
