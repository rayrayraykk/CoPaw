...
async def get_mcp_policy(
    ...
    agent = await _agent_for_request(request)
    return _mcp_service(agent).get_policy(client_key)
...
async def list_mcp_clients(request: Request) -> List[MCPClientInfo]:
    ...
    agent = await _agent_for_request(request)
    return _mcp_service(agent).list_clients()
...
async def get_mcp_client(
    ...
    agent = await _agent_for_request(request)
    card = _mcp_service(agent).load_card(client_key)
    return _mcp_service(agent).build_info_from_card(card)
