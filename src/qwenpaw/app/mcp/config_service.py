...
class MCPConfigService:
    ...
    def load_card(self, client_key: str) -> DriverCard:
        return self._driver_config.load_card(
            client_key,
            protocol=MCP_PROTOCOL,
        )

    def list_cards(self) -> list[DriverCard]:
        return self._driver_config.list_cards(protocol=MCP_PROTOCOL)

    def build_info_from_card(self, card: DriverCard) -> MCPClientInfo:
    ...
    def list_clients(self) -> list[MCPClientInfo]:
        return [self.build_info_from_card(card) for card in self.list_cards()]

    async def list_tools(self, client_key: str) -> list[MCPToolInfo]:
        card = self.load_card(client_key)
        if not card.enabled:
            return []
        try:
    ...
    def get_policy(self, client_key: str) -> MCPAccessPolicy:
        return mcp_access_policy_from_card(self.load_card(client_key))
    ...
    async def create_client(
        ...
        if credential.secrets:
            self._driver_config.credential_store.put(credential)
        else:
            self._driver_config.credential_store.delete(credential.ref)
        await self._driver_config.save_card(card)
        return self.build_info_from_card(card)
    ...
    async def update_client(
        ...
        existing_card = self.load_card(client_key)
        existing_info = self.build_info_from_card(existing_card)
        ...
        existing_credential = self._driver_config.load_optional_credential(
            mcp_credential_ref(client_key),
        )
...
def ensure_mcp_display_name_unique(
