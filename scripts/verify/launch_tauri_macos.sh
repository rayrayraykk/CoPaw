...
PORT_FILE="${RUNNER_TEMP:-/tmp}/qwenpaw-desktop-port"
rm -f "$PORT_FILE"
export QWENPAW_DESKTOP_PORT_FILE="$PORT_FILE"
...
# 5. Wait for Rust Core to publish its port at the verifier-owned path and
#    respond.
PORT=""
for i in $(seq 1 60); do
  ...
  if [ "$i" = "60" ]; then
    ...
    echo "[debug] Verifier temp directory contents:"
    ls -la "$(dirname "$PORT_FILE")" 2>/dev/null || echo "  (missing)"
